<template>
  <div class="essay-container-title">
    真的会写一些随笔 !!!
  </div>
...
import router from '@/router';
  ...
  const end = ref<number>(6)

  const showEssay = computed(()=>{
  return essay.value.slice(essay.value.length-end.value, essay.value.length-start.value).reverse()
  })
  const leftPage = ()=>{
  if(start.value >= 6){
    start.value -= 6
  ...
  const rightPage = ()=>{
  if(end.value < essay.value.length){
    start.value += 6
    if(end.value+6 > essay.value.length){
      end.value = essay.value.length
    }
    else {
      end.value += 6
    }
